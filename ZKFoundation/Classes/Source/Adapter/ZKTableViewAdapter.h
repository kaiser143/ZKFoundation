 ...
 */
// 读取侧 getter 为 isSelfSizingEstimateEnabled，符合 UIKit BOOL 命名习惯。
@property (nonatomic, assign, getter=isSelfSizingEstimateEnabled) BOOL selfSizingEstimateEnabled;
...
/** 设置动态计算行高的 block，常与 UITableView+FDTemplateLayoutCell 等方案配合；使用后建议将 estimatedRowHeight 设为 0，可通过 selfSizingEstimateEnabled 统一控制 */
