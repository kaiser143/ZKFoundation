 ...
 */
- (void)textView:(ZKTextView *)textView newHeightAfterTextChanged:(CGFloat)height;
 ...
 */
- (BOOL)textViewShouldReturn:(ZKTextView *)textView;
 ...
 */
- (BOOL)textView:(ZKTextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text originalValue:(BOOL)originalValue;
 ...
 */
- (void)textView:(ZKTextView *)textView didPreventTextChangeInRange:(NSRange)range replacementText:(NSString *)replacementText;
...
/**
 *  在使用 maximumTextLength 功能的时候，是否应该把文字长度按照 [NSString (ZKAdd) lengthWhenCountingNonASCIICharacterAsTwo] 的方法来计算。
 *  默认为 NO。
 */
