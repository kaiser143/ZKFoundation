...
@interface _KAILoadingSpinnerTimingFunction : NSObject
...
@implementation _KAILoadingSpinnerTimingFunction
...
@interface ZKLoadingSpinner () <CAAnimationDelegate>
...
@implementation ZKLoadingSpinner
...
- (void)setup {
    ...
    self.rotationDirection     = ZKRotationDirectionClockwise;
    ...
    self.drawTimingFunction = [_KAILoadingSpinnerTimingFunction easeInOut];
    ...
    if (nil != [ZKLoadingSpinner appearance].colorSequence) {
        self.colorSequence = [ZKLoadingSpinner appearance].colorSequence;
    }
...
- (void)setRotationDirection:(ZKRotationDirection)rotationDirection {
    _rotationDirection = rotationDirection;

    switch (rotationDirection) {
        case ZKRotationDirectionCounterClockwise:
            self.layer.affineTransform = CGAffineTransformScale(CGAffineTransformIdentity, -1, 1);
            break;

        case ZKRotationDirectionClockwise:
        default:
            self.layer.affineTransform = CGAffineTransformScale(CGAffineTransformIdentity, 1, 1);
            break;
    }
...
- (void)addAnimationsToLayer:(CAShapeLayer *)layer reverse:(BOOL)reverse rotationOffset:(CGFloat)rotationOffset {
    ...
    if (reverse) {
        ...
        rotationAnimation.timingFunction    = [_KAILoadingSpinnerTimingFunction linear];
