...
@protocol ZKTextFieldDelegate <UITextFieldDelegate>
 ...
 */
- (void)textField:(ZKTextField *)textField didPreventTextChangeInRange:(NSRange)range replacementString:(NSString *)replacementString;
...
@property(nonatomic, weak) id<ZKTextFieldDelegate> delegate;
...
/**
 *  在使用 maximumTextLength 功能的时候，是否应该把文字长度按照 [NSString lengthWhenCountingNonASCIICharacterAsTwo] 的方法来计算。
 *  默认为 NO。
 */
