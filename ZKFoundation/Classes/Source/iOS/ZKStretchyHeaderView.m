...
@interface ZKStretchyHeaderView ()
...
@interface ZKStretchyHeaderContentView : UIView
@end

@implementation ZKStretchyHeaderView
...
- (void)setupView {
    ...
    self.contentAnchor = ZKStretchyHeaderViewContentAnchorTop;
...
- (void)setupContentView {
    _contentView = [[ZKStretchyHeaderContentView alloc] initWithFrame:self.bounds];
    [self kai_transplantSubviewsToView:_contentView];
...
- (void)setExpansionMode:(ZKStretchyHeaderViewExpansionMode)expansionMode {
    _expansionMode = expansionMode;
    [self.scrollView kai_layoutStretchyHeaderView:self
...
- (void)didMoveToWindow {
    ...
    if (@available(iOS 11.0, *)) { // it has to be used like this :/
        [self.scrollView kai_fixZPositionsForStretchyHeaderView:self];
    }
}
...
- (void)observeValueForKeyPath:(nullable NSString *)keyPath
    ...
    if (object == self.scrollView) {
        ...
        if (!self.arrangingSelfInScrollView && self.manageScrollViewSubviewHierarchy) {
            ...
            [self.scrollView kai_arrangeStretchyHeaderView:self];
...
- (void)layoutContentViewIfNeeded {
    ...
    switch (self.contentAnchor) {
        case ZKStretchyHeaderViewContentAnchorTop: {
            contentViewTop = self.contentInset.top;
            break;
        }
        case ZKStretchyHeaderViewContentAnchorBottom: {
...
@implementation ZKStretchyHeaderContentView

- (void)layoutSubviews {
    [super layoutSubviews];
    if ([self.superview isKindOfClass:[ZKStretchyHeaderView class]]) {
        [(ZKStretchyHeaderView *)self.superview contentViewDidLayoutSubviews];
    }
}
