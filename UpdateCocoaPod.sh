...
currentVersionString=`grep -E 's.version.*=' ZKFoundation.podspec`
...
lineNumber=`grep -nE 's.version.*=' ZKFoundation.podspec | cut -d : -f1`

sed -i "" "${lineNumber}s/${currentVersion}/${newVersion}/g" ZKFoundation.podspec

echo "Current version: ${currentVersion}, New version: ${newVersion}"

#Push ZKFoundation.podspec

# git add ZKFoundation.podspec
